  ...
  const getSavedData = async (totalCoins = allCoins) => {
    try {
      const data = await fetch(
        `https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=${currency}&ids=${totalCoins.join(
          ","
        )}&order=${sortBy}&sparkline=false&price_change_percentage=1h%2C24h%2C7d&x_cg_pro_api_key=hifxrsmm2TzEqGmg2Rtdx4Bk`,
        {
          headers: {
