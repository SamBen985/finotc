...
export const CryptoProvider = ({ children }) => {
  ...
  const getCryptoData = async () => {
    ...
    try {
      const data = await fetch(
        `https://pro-api.coingecko.com/api/v3/coins/markets?vs_currency=${currency}&ids=${coinSearch}&order=${sortBy}&per_page=${perPage}&page=${page}&sparkline=false&price_change_percentage=1h%2C24h%2C7d&x_cg_pro_api_key=hifxrsmm2TzEqGmg2Rtdx4Bk`,
        {
          headers: {
  ...
  const getCoinData = async (coinid) => {
    ...
    try {
      const data = await fetch(
        `https://pro-api.coingecko.com/api/v3/coins/${coinid}?localization=false&tickers=false&market_data=true&community_data=false&developer_data=true&sparkline=false&x_cg_pro_api_key=hifxrsmm2TzEqGmg2Rtdx4Bk`,
        {
          headers: {
            "x-cg-pro-api-key": "CG-hifxrsmm2TzEqGmg2Rtdx4Bk",
          },
